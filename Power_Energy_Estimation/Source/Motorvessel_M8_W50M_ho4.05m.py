...
data_moteur = {"P_installed": 1750.0,
               "L_w": 3.0 ,
               "C_year":2010,
               "Typvessel":'Inland'
              }             
...
plt.grid(True)
plt.legend()
plt.title('Riverboat M8: L$_s$ = 110 m, B$_s$ = 11.4 m, T$_s$ = 2.7 m')
plt.savefig('Fig_11.pdf',bbox_inches = 'tight', dpi=600, format='pdf')

#Plot of Total resistance (kN)
realvit = [1.5,2,2.5,3,3.5]
realres = [7.3,12.44,18.9,26.55,37.7]
plt.interactive(True)
plt.figure()
plt.plot(velocity[:],R_tot[:],'r-',label='Total resistance')
plt.plot(realvit,realres,'ro',label='TU Delft method D.Gkioka (2023)')
plt.xlabel('Velocity(m/s)')
...
realpow = [40.74,93,176.9,297.8,492.9]
plt.interactive(True)
plt.figure()
...
plt.plot(realvit,realpow,'ro',label='TU Delft method D.Gkioka (2023)')
plt.xlabel('Velocity(m/s)')
plt.ylabel('Power (kW)')
plt.grid(True)
plt.legend()
